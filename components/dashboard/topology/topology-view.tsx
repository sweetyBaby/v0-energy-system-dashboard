...
const ICON_TARGET_WORLD = 58
const LABEL_FLOOR_PX = 22 // 标签最小屏幕字号 CSS px（文字优先，确保正常可读）
const FIELD_FLOOR_PX = 19 // 字段最小屏幕字号 CSS px
...
export function TopologyView({ doc, resolveNav, onNavigate, fitZoomCap, fullscreen = false }: TopologyViewProps) {
  ...
  const fitToView = () => {
    ...
    labelFloorRef.current = clamp((LABEL_FLOOR_PX * effDpr) / (ENGINE_BASE_FONT * z), 1, TEXT_FLOOR_CAP)
    fieldFloorRef.current = clamp((FIELD_FLOOR_PX * effDpr) / (ENGINE_BASE_FONT * 0.92 * z), 1, TEXT_FLOOR_CAP)
    applyScale(1)
    engine.fitView(fitCapRef.current)
