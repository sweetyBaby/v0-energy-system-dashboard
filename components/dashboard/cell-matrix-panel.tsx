...
const buildCells=():Cell[]=>Array.from({length:50},(_,i)=>{
  const id=i+1
  const bv=3.32+Math.sin(id*.71)*.04+(id%7===0?.07:0)+(id%13===0?-.06:0)
  ...
  return{id,voltage:+bv.toFixed(3),temp1:+t1.toFixed(1),temp2:+t2.toFixed(1),temp3:+t3.toFixed(1),avgTemp:+((t1+t2+t3)/3).toFixed(1)}
})
  ...
  return(
    <div className="flex items-start gap-1.5">
          ...
          <div key={ri} className="flex gap-1">
            {row.map((id,ci)=>id!=null?(
              <button key={id} onClick={()=>onClk(id)}
                className={`relative h-10 w-10 rounded transition-all ${sel===id?"scale-105 ring-2 ring-white ring-offset-1 ring-offset-[#0d1233]":"hover:scale-105 hover:ring-1 hover:ring-white/50"}`}
                style={{backgroundColor:isV?voltColor(cellMap[id].voltage,vs.min,vs.max,vs.avg):tempColor(cellMap[id].avgTemp)}}>
                <div className="flex h-full flex-col items-center justify-center">
                  <div className="text-[7px] leading-none" style={{color:fg(isV?voltColor(cellMap[id].voltage,vs.min,vs.max,vs.avg):tempColor(cellMap[id].avgTemp)),opacity:.6}}>#{id}</div>
                  <div className="text-[8px] font-bold leading-none" style={{color:fg(isV?voltColor(cellMap[id].voltage,vs.min,vs.max,vs.avg):tempColor(cellMap[id].avgTemp))}}>
                    {isV?`${cellMap[id].voltage.toFixed(3)}`:`${cellMap[id].avgTemp.toFixed(1)}°`}
                  </div>
                </div>
                {sel===id&&<div className="absolute inset-0 rounded" style={{background:"rgba(255,255,255,0.1)"}}/>}
              </button>
            ):(
              <div key={`e${ri}${ci}`} className="h-10 w-10"/>
            ))}
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col items-center self-stretch py-0.5">
        <span className="text-[8px] text-[#7b8ab8]">{isV?"高":"34"}</span>
        ...
        <span className="text-[8px] text-[#7b8ab8]">{isV?"低":"22"}</span>
      </div>
    </div>
  )
}
...
export function CellMatrixPanel(){
  ...
  const cStats=useMemo(()=>{
    ...
    return{
      vRange:(Math.max(...v)-Math.min(...v)).toFixed(3),vStd:std(v,vm).toFixed(4),vAvg:vm.toFixed(3),
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={[...cells].sort((a,b)=>a.id-b.id).map(c=>({id:c.id,voltage:c.voltage}))} margin={{top:8,right:12,left:-12,bottom:20}}>
                ...
                <Tooltip contentStyle={TS} labelStyle={{color:"#7b8ab8"}}
                  formatter={(v:number)=>[`${v.toFixed(3)} V`,zh?"电压":"Voltage"]}
          ...
          <div className="relative flex w-[780px] max-w-[90vw] flex-col gap-3 rounded-2xl border border-[#00d4aa]/30 bg-[#0d1233] p-4 shadow-2xl">
            ...
            <div className="flex gap-4">
              ...
              <div className="grid shrink-0 grid-cols-1 gap-1.5" style={{width:100}}>
                {[
                  {label:zh?"电压":"Volt",  value:`${cmap[sel]?.voltage.toFixed(3)} V`,  color:"#7dd3fc"},
                ...
                <div className="h-48">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={hist} margin={{top:6,right:12,left:-20,bottom:0}}>
                      ...
                      <Tooltip contentStyle={TS} labelStyle={{color:"#7b8ab8"}} formatter={(v:number)=>[`${v.toFixed(3)} V`,zh?"电压":"Voltage"]}/>
