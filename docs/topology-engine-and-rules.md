...
> `public/topology-sample.json`、`public/topo/icons/* + icon-manifest.json`、
> `scripts/build-topo-engine.mjs`、`scripts/build-topo-icons.mjs`、`vendor/topo-editor/*`。
>
...
app/dashboard/page.tsx (realtime tab, 12 列栅格)
  └─ ProjectTopologyPanel            卡片外壳 + 全屏开关 + 节点点击路由；取数走接缝层
       └─ TopoCanvas                 数据层：调 loadDoc 拉布局 + 每 2s 调 makeSignals 合并信号
            └─ TopologyView          视图层：canvas 容器 + 缩放/拖动/自适应/命中
                 └─ lib/topo/engine.ts   渲染引擎：真正往 canvas 上画（"哑"渲染器）
```

取数接缝（`lib/topo/topo-api.ts`，前后台交互唯一入口，见 `docs/拓扑前后台接口定义.md`）：

```
TOPOLOGY_USE_MOCK=true（现状）          后端就绪：置 false，只改本文件
  fetchTopologyLayout() → /topology-sample.json        →  GET /topology/layout?projectId=…
  makeTopologySignals() → (t)=>makeMockSignals(t)      →  GET /topology/signals?projectId=…（轮询）
```

数据流（单向）：

```
TopoCanvas 调 loadDoc() 拿 baseDoc + 每 2s 调 makeSignals(t)
...
| 文件 | 职责 |
|---|---|
| `project-topology-panel.tsx` | 卡片外壳；经接缝层取数（`loadDoc`/`makeSignals`）；全屏；节点点击 → 路由/切 tab |
| `topo-api.ts` | **取数接缝（前后台交互唯一入口）**：mock↔API 切换只改此文件；见 `docs/拓扑前后台接口定义.md` |
| `topo-canvas.tsx` | 调 `loadDoc` 拉布局 + 轮询 `makeSignals`；累积合并；纵览/运行状态页共用（仍兼容 `url` 静态用法） |
...
| **图标（新增/改图/删除）** | 运营端加了新设备 type、换了某图标、或删了图标 | **同步图标** → §9 |
...
## 9. 图标同步（public/topo/icons）

**核心：拓扑 JSON 只按 `node.type` 引用图标，图标字节与布局解耦。** 图标是**构建期 vendored 静态资源**——本项目**不让后台托管图标**。

### 9.1 链路

```
运营端「元素库包」(dlAllIconsZip)：icons/ 全量文件 + icons.js(ICON_DATA) + element-library.json(version)
   │  解压进 vendor/topo-editor/
   ▼  node scripts/build-topo-icons.mjs   （源优先级：vendor/.../icons/ > vendor/.../icons.js）
public/topo/icons/{type}.{svg|png}        每个图标一个文件（svg 可 diff、png 二进制可见）
public/topo/icon-manifest.json            { version, count, paths:{type→文件名} }
   ▼  loadTopoIcons(usedTypes)            按画布用到的 type 走 /topo/icons/{file}?v=version 加载（逐图标缓存）
engine.setIcons(imgs) → IMGS[type]        缺图标 → 画蓝底方块+type 文字兜底（engine.ts:1250），不崩
```

**为什么用文件路径而非内联 dataURL**：可审查的 git diff、按图标的缓存粒度、无 base64 +33% 膨胀、可只加载画布用到的 type。dataURL 仅在「必须单文件自包含/离线」时才更优。

> **现状（重要）**：`public/topo/icons/*` 与 `icon-manifest.json` 已生成并提交（34 个，version=2.0.0），应用运行时只依赖它们。最初用于引导生成的 `public/topo-icons.json`（旧的全量内联 dataURL）**已删除**。
> 因此**在把运营端正式「元素库包」vendor 进 `vendor/topo-editor/`（至少要有 `icons/` 或 `icons.js`）之前，`build-topo-icons.mjs` 会报「找不到图标源」**——这是预期行为，不影响运行时（已有产物仍在）。脚本仍保留对 `public/topo-icons.json` 的兜底读取，若临时需要可从 git 历史恢复该文件。

### 9.2 运营端改图标时怎么做

```bash
# 1) 运营端导出「元素库包」，解压进 vendor/topo-editor/（至少要有 icons/ 或 icons.js；有 element-library.json 更好，带版本号）
node scripts/build-topo-icons.mjs --check     # 2) 干跑：看源、版本、将写出的文件
node scripts/build-topo-icons.mjs             # 3) 写入 public/topo/icons/ + icon-manifest.json
git diff public/topo/                          # 4) 核对：svg 文本可读、png 二进制变更可见，能看出改了哪个
# 5) dev 验证（§6）：图标正常显示、无兜底蓝方块
```
...
| 工况 | 不重跑脚本的后果 | 处理 |
|---|---|---|
| 新增图标（新 type） | 新节点显示兜底蓝方块（不崩） | 重跑脚本，新 type 自动并入清单 |
| 修改已有图标 | 显示旧图（且浏览器可能再缓存一层） | 重跑脚本；**版本号变了** `?v=` 自动失效缓存 |
| 删除图标 | 仍被引用 → 兜底方块 | 重跑脚本会从 `public/topo/icons/` 清掉（脚本先清空目录再重写） |
| 重命名 type | type 是契约 key | 运营端布局 JSON 与图标 type 要一起改 |

> **版本号 / 缓存失效**：清单 `version` 取自 `element-library.json` 的 `library.version`（缺省读既有清单 / `--version=` 参数 / `0.0.0`），与布局 JSON 的 `meta.libraryRef.version` 同源。改图标务必让运营端 bump 版本，`loadTopoIcons` 的 `?v=` 才会绕开浏览器缓存。

### 9.3 接 API 阶段

图标仍是前端静态资源（不碰后台）。取数接缝 `lib/topo/topo-api.ts` 只负责布局/信号两条数据流；图标走 `loadTopoIcons` 不变。
...
- 前后台数据接口：`docs/拓扑前后台接口定义.md`（布局 + 实时信号两条流）
